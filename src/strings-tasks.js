...
function removeFirstOccurrences(str, value) {
  ...
  const SecondPart = str.slice(end);
...
function formatTime(minutes, seconds) {
  const formatedMinutes =
    String(minutes).length === 1 ? String(minutes).padStart(2, '0') : minutes;
  const formatedSeconds =
    String(seconds).length === 1 ? String(seconds).padStart(2, '0') : seconds;
  return formatedMinutes.concat(':', formatedSeconds);
}
...
function isPalindrome(str) {
  const joined = str.replaceAll(/\W/g, '').toLowerCase();
...
function invertCase(str) {
  ...
  for (let i = 0; i <= str.length; i += 1) {
    if (str.charAt(i) === str.charAt(i).toUpperCase()) {
      invertedCaseStr += str.charAt(i).toLowerCase();
    } else {
      invertedCaseStr += str.charAt(i).toUpperCase();
    }
  }
